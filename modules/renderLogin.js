...
export const initRenderLoginForm = () => {
  const containerFormsElement = document.querySelector(
    ...
    enterFormButton.addEventListener("click", () => {
      login({
        ...
        .then((responseData) => {
          ...
          setLoginUser(
            `${responseData.user.name} (${responseData.user.login})`,
          );
        })
        .then(() => {
    ...
    registFormButton.addEventListener("click", () => {
      registration({
        ...
        .catch((error) => {
          if (error.message === "Плохой запрос") {
            alert(
              "Пользователь с таким логином уже сущетсвует, скорректируйте ввод",
            );
            return;
          } else {
            alert("Кажется, у вас сломался интернет, попробуйте позже");
